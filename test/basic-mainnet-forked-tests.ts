...
describe('Basic Forked Mainnet Tests', function () {
  ...
  describe('On-Chain Quote Testing', function () {
    it('Should process atomic balancer swap correctly', async function () {
      ...
      const sendAmountNumber = getLoopingSendAmount(
        collTokenFromBorrowerNumber,
        loanPerColl,
        collTokenInDexPool,
        loanTokenInDexPool,
        swapFee
      )
      const sendAmount = ethers.BigNumber.from(Math.floor(sendAmountNumber * PRECISION))
        .mul(ONE_WETH)
        .div(PRECISION)
      ...
      const data = ethers.utils.defaultAbiCoder.encode(['bytes32', 'uint256', 'uint256'], [poolId, minSwapReceive, deadline])
      await firstLenderVault
        .connect(borrower)
        .borrowWithOnChainQuote(onChainQuote, false, sendAmount, balancerV2Looping.address, data)
  ...
  describe('Compartment Testing', function () {
    it('Should proceed Curve staking correctly', async () => {
      ...
      const CurveStakingCompartment = await ethers.getContractFactory('CurveStakingCompartment')
      await CurveStakingCompartment.connect(vaultOwner)
      const curveStakingCompartment = await CurveStakingCompartment.deploy(lenderVaultFactory.address)
      await curveStakingCompartment.deployed()

      const CompartmentFactory = await ethers.getContractFactory('CollateralCompartmentFactory')
      await CompartmentFactory.connect(vaultOwner)
      const compartmentFactory = await CompartmentFactory.deploy([curveStakingCompartment.address])
      await compartmentFactory.deployed()
      ...
      const crvInstance = new ethers.Contract(
        crvTokenAddress,
        [
          {
            name: 'balanceOf',
      ...
      expect(await crvInstance.balanceOf(borrower.address)).to.equal(BigNumber.from(1))

      const borrowCompartment = await compartmentFactory.createCompartment(
        curveStakingCompartment.address,
        lenderVault.address,
        borrower.address,
        crvTokenAddress,
        1,
