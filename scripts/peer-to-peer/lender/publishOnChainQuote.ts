  ...
  if (hardhatNetworkName in jsonConfig) {
    ...
    try {
      ...
      switch (answer.toLowerCase()) {
        case 'y':
          await publishOnChainQuote(signer, hardhatNetworkName, jsonConfig)
...
async function publishOnChainQuote(signer: any, hardhatNetworkName: string, jsonConfig: any) {
  ...
  for (let onChainQuote of jsonConfig[hardhatNetworkName]['onChainQuotes']) {
    ...
    logger.log(`Publishing on-chain quote...`)
    const tx = await quoteHandler.connect(signer).publishOnChainQuote(onChainQuote)
    const receipt = await tx.wait()
    const event = receipt.events?.find(x => {
      return x.event === 'OnChainQuotePublished'
    })
    const quoteHash = event?.args?.['onChainQuoteHash']
    logger.log(`Published on-chain quote with quote hash '${quoteHash}'.`)
  }
}
